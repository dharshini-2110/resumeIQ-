...
const menuItems = [
  ...
  { icon: ArrowLeftRight, label: "Resume Versions", path: "/resume-versions" },
  { icon: Trophy, label: "Career Momentum", path: "/gamification" },
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const onboardingQuery = trpc.onboarding.get.useQuery(undefined, { enabled: Boolean(user) });
  ...
  useEffect(() => {
    if (user && onboardingQuery.data && !onboardingQuery.data.completed && location !== "/onboarding") {
...
function DashboardLayoutContent({
  ...
  const setPasswordMutation = trpc.auth.setPassword.useMutation({
    onSuccess: () => {
      toast.success("Password enabled. You can now sign in with your email and password.");
      setPasswordDialogOpen(false);
      setNewPassword("");
      setConfirmNewPassword("");
    },
    onError: error => toast.error(error.message || "Unable to set your password."),
  });
  ...
  return (
    <>
      <div className="relative" ref={sidebarRef}>
        ...
        <Dialog open={passwordDialogOpen} onOpenChange={setPasswordDialogOpen}>
          <DialogContent className="border-white/10 bg-card/95 backdrop-blur-xl sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Enable email sign-in</DialogTitle>
              <DialogDescription>
                Create a password for <strong>{user?.email}</strong>. Your Manus OAuth sign-in will continue to work too.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <Label htmlFor="new-account-password">New password</Label>
                <Input id="new-account-password" type="password" minLength={8} maxLength={128} value={newPassword} onChange={event => setNewPassword(event.target.value)} autoComplete="new-password" placeholder="At least 8 characters" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="confirm-account-password">Confirm password</Label>
                <Input id="confirm-account-password" type="password" minLength={8} maxLength={128} value={confirmNewPassword} onChange={event => setConfirmNewPassword(event.target.value)} autoComplete="new-password" placeholder="Repeat your password" />
              </div>
              <Button onClick={savePassword} disabled={setPasswordMutation.isPending || newPassword.length < 8 || confirmNewPassword.length < 8} className="w-full">
                {setPasswordMutation.isPending ? "Saving…" : "Enable email sign-in"}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <SidebarInset>
        ...
        <main className="flex-1 p-4 lg:p-6 aurora-bg intelligence-grid min-h-screen">
